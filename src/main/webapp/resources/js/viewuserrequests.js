...
async function viewReq(event) {
	...
	let idx = document.getElementById('username').value;
	let user = users[idx];
	
	let req = await fetch('http://localhost:8080/Project001/api/userrequests', {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json'
		},
	...
	for(let i = 0; i < reimbs.length; i++) {
		if(!(reimbs[i].resolverId)) {
			...
			req = await fetch('http://localhost:8080/Project001/api/resolver', {
				method: 'POST',
				headers: {
					'Content-Type' : 'application/json'
				},
