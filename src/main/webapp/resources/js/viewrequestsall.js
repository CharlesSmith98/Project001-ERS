...
async function viewReq(e) {
	...
	if(filter == 'Pending') {
		url = 'http://localhost:8080/Project001/api/allpending';
	} else {
		url = 'http://localhost:8080/Project001/api/allresolved';
	}
	...
	for(let i = 0; i < res.length; i++) {
		...
		if(Number(res[i].resolverId)) {
			temp = await fetch('http://localhost:8080/Project001/api/resolver', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
